    ...
    async function loginUser(event) {
        ...
        if(email === savedEmail){
            ...
            if(password ===savedPass){   
                var loggedInStatus = localStorage.setItem("loggedInStatus", true)
                var savedPass = localStorage.getItem("name")
                setTimeout(() => {
                    setLoading(false)
                    setSuccess(true)
                  }, 2000);
...
<section className="bg-gray-50 dark:bg-gray-900">
  ...
  {loading && <div className='absolute bg-[rgba(0,0,0,0.5)] w-[60vw] h-[60vh] flex justify-center items-center' role="status">
    <p className='text-white mr-1 text-[20px]'>Checking credentials. Please wait &nbsp;&nbsp;</p>
  ...
  {success && <div id="toast-success" class="flex items-center p-4 mb-4 w-full max-w-xs text-gray-500 bg-white rounded-lg shadow dark:text-gray-400 dark:bg-gray-800" role="alert">
      ...
      <div class="ml-3 text-sm font-normal">Welcome, {name}! Logging you in.</div>
    </div>}
...
{error && <div id="toast-warning" class="flex items-center p-4 w-full max-w-xs text-gray-500 bg-white rounded-lg shadow dark:text-gray-400 dark:bg-gray-800" role="alert">
    ...
    <div class="ml-3 text-sm font-normal">Credentials didn't match!</div>
      ...
      <div className="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
          <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
              ...
              <form className="space-y-4 md:space-y-6" onSubmit={loginUser}>
                  ...
                  >
Sign in                  </button>                  <p className="text-sm font-light text-gray-500 dark:text-gray-400">
                      Don’t have an account yet?  <NavLink to='/register'><a className="font-medium text-primary-600 hover:underline dark:text-primary-500">Sign up</a></NavLink>
                  </p>
              </form>
          </div>
      </div>
